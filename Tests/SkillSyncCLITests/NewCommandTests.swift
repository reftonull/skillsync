...
extension BaseSuite {
  ...
  struct NewCommandTests {
    @Test
    func createsNewSkill() async throws {
      let fileSystem = InMemoryFileSystem(
        homeDirectoryForCurrentUser: URL(filePath: "/Users/blob", directoryHint: .isDirectory)
      )

      try await assertCommand(
        ["new", "pdf", "--description", "Extract and summarize PDF text."],
        stdout: {
          """
    ...
    @Test
    func duplicateSkillThrowsActionableError() async throws {
      let fileSystem = InMemoryFileSystem(
        homeDirectoryForCurrentUser: URL(filePath: "/Users/blob", directoryHint: .isDirectory)
      )
